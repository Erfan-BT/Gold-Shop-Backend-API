...
import { ReturnRequestDto } from "../validation/return.validation.js";
...
class ReturnService {
    ...
    async getReturnRequest (userId : number, returnId : number)
    : Promise<ReturnRequest> {
        // Get Request
    ...
    async createReturnRequest (userId : number, returnRequestBody : ReturnRequestDto)
    : Promise<{
        ...
        if (!order)
            throw new NotFoundError(`Order Not Found { Order-Number : ${returnRequestBody.orderNumber} }`)
        // Check Order Status
    ...
    : Promise<void> {
        ...
        if (!returnRequest)
            throw new NotFoundError(`Return Request Not Found { ID : ${returnId} }`)
        if (returnRequest.returnTrackingCode !== null)
            throw new ConflictError('Tracking Code Already Registered')
        if (returnRequest.status !== ReturnStatus.PARTIALLY_APPROVED && returnRequest.status !== ReturnStatus.APPROVED)
    ...
    : Promise<void> {
        ...
        // Cancel
        if (!(await returnRepository.userCancelReturnRequest(returnId, userId, reason)))
